...
# تابع دانلود دیتابیس بانک ملی از لینک (در صورت نیاز، بررسی کنید که فایل وجود ندارد)
def download_melli_bank_csv():
    if not os.path.exists(MELLI_BANK_CSV):
        try:
            r = requests.get(MELLI_BANK_URL)
            r.raise_for_status()
            with open(MELLI_BANK_CSV, "wb") as f:
                f.write(r.content)
            logger.info("Melli Bank CSV downloaded successfully.")
